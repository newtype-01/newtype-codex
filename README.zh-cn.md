...
newtype for Codex 则更克制。Codex 插件目前会自动安装 skills，但不会在安装时执行 post-install 脚本，也不会自动注册 custom agents。自定义 subagents 仍然是 `.codex/agents/*.toml` 文件。因此本仓库同时提供两层：插件安装 newtype skills，安装脚本写入 newtype custom agent 模板。
...
这一步目前不能由 Codex plugin 安装器自动完成：插件安装不会执行任意 setup 脚本，Codex 也还不会把插件内置的 custom agents 自动注册到 `~/.codex/agents/`。
